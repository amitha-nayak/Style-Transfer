...
def make_tuning_step(model, optimizer, target_representation, should_reconstruct_content, content_feature_maps_index, style_feature_maps_indices):
    ...
    def tuning_step(optimizing_img):
        ...
        if should_reconstruct_content:
            ...
            current_representation = [utils.gram_matrix(fmaps) for i, fmaps in enumerate(
                set_of_feature_maps) if i in style_feature_maps_indices]
        ...
        if should_reconstruct_content:
            ...
            for gram_gt, gram_hat in zip(target_representation, current_representation):
                loss += (1 / len(target_representation)) * \
...
def reconstruct_image_from_representation(config):
    ...
    dump_path = os.path.join(
        config['output_img_dir'], ('c' if should_reconstruct_content else 's') + '_reconstruction_' + config['optimizer'])
    dump_path = os.path.join(dump_path, os.path.basename(config['content_img_name']).split(
        '.')[0] if should_reconstruct_content else os.path.basename(config['style_img_name']).split('.')[0])
    ...
    #
    if should_reconstruct_content:
        target_content_representation = set_of_feature_maps[content_feature_maps_index_name[0]].squeeze(
            axis=0)
        ...
        target_style_representation = [utils.gram_matrix(fmaps) for i, fmaps in enumerate(
            set_of_feature_maps) if i in style_feature_maps_indices_names[0]]
        if should_visualize_representation:
    ...
    if config['optimizer'] == 'adam':
        ...
        tuning_step = make_tuning_step(neural_net, optimizer, target_representation, should_reconstruct_content,
                                       content_feature_maps_index_name[0], style_feature_maps_indices_names[0])
        for it in range(num_of_iterations[config['optimizer']]):
            ...
            with torch.no_grad():
                ...
                utils.save_and_maybe_display(
                    optimizing_img, dump_path, config, it, num_of_iterations[config['optimizer']], should_display=False)
        ...
        def closure():
            ...
            loss = 0.0
            if should_reconstruct_content:
                loss = torch.nn.MSELoss(reduction='mean')(target_content_representation, neural_net(
                    optimizing_img)[content_feature_maps_index_name[0]].squeeze(axis=0))
            else:
                current_set_of_feature_maps = neural_net(optimizing_img)
                current_style_representation = [utils.gram_matrix(fmaps) for i, fmaps in enumerate(
                    current_set_of_feature_maps) if i in style_feature_maps_indices_names[0]]
        ...
        optimizer = torch.optim.LBFGS(
            (optimizing_img,), max_iter=num_of_iterations[config['optimizer']], line_search_fn='strong_wolfe')
